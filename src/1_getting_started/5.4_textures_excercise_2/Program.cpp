...
int main()
{
    ...
    SDL_GPUSamplerCreateInfo clampSamplerInfo = {
    ...
    SDL_GPUSampler* clampSampler = SDL_CreateGPUSampler(graphicsDevice, &clampSamplerInfo);

    SDL_GPUSamplerCreateInfo repeatSamplerInfo = {
        .min_filter = SDL_GPU_FILTER_LINEAR,
        .mag_filter = SDL_GPU_FILTER_LINEAR,
    ...
    SDL_GPUSampler* repeatSampler = SDL_CreateGPUSampler(graphicsDevice, &repeatSamplerInfo);
    ...
    while (!_shouldQuit)
    {
        ...
        if (swapchainTexture != NULL)
        {
            ...
            SDL_GPUTextureSamplerBinding textureSamplers[] = {
                { .texture = containerTexture, .sampler = clampSampler },
                { .texture = awesomefaceTexture, .sampler = repeatSampler },
            };
    ...
    pipeline->Release();
    SDL_ReleaseGPUSampler(graphicsDevice, clampSampler);
    SDL_ReleaseGPUSampler(graphicsDevice, repeatSampler);
