...
int main()
{
    ...
    std::optional<Shader> fragmentShader = 
        Shader::FromSPV(graphicsDevice, "shaders/fragment_shader.spv", { .ShaderStage = SDL_GPU_SHADERSTAGE_FRAGMENT, .NumSamplers = 1 });
    ...
    Image* image = LoadImage("assets/container.png");
    ...
    SDL_GPUTexture* texture = SDL_CreateGPUTexture(graphicsDevice, &textureInfo);
    ...
    gpuUploader.AddTextureData(image->Data, image->Width, image->Height, texture);
    ...
    while (!_shouldQuit)
    {
        ...
        if (swapchainTexture != NULL)
        {
            ...
            SDL_GPUTextureSamplerBinding textureSampler = { .texture = texture, .sampler = sampler };
    ...
    SDL_ReleaseGPUTexture(graphicsDevice, texture);
    pipeline->Release();
