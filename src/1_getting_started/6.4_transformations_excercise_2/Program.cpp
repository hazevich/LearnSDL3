...
int main()
{
    ...
    while (!_shouldQuit)
    {
        ...
        if (swapchainTexture != NULL)
        {
            ...
            transformation1 = glm::rotate(transformation1, _time.TotalTime, glm::vec3(0.0f, 0.0f, 1.0f));
            SDL_PushGPUVertexUniformData(commandBuffer, 0, glm::value_ptr(transformation1), sizeof(transformation1));
            SDL_DrawGPUIndexedPrimitives(renderPass, 6, 1, 0, 0, 0);

            glm::mat4 transformation2 = glm::mat4(1.0f);
            transformation2 = glm::translate(transformation2, glm::vec3(-0.5f, 0.5f, 0.0f));
            float scale = glm::sin(_time.TotalTime);
            transformation2 = glm::scale(transformation2, glm::vec3(scale));
            SDL_PushGPUVertexUniformData(commandBuffer, 0, glm::value_ptr(transformation2), sizeof(transformation2));
            SDL_DrawGPUIndexedPrimitives(renderPass, 6, 1, 0, 0, 0);
